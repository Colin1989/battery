...
func TestPoolRouterActor_Receive_AddRoute(t *testing.T) {
	...
	c := new(mockContext)
	c.On("Envelope").Return(actor.WrapEnvelope(&AddRoutee{PID: p1}))
	c.On("Watch", p1).Once()
...
func TestPoolRouterActor_Receive_RemoveRoute(t *testing.T) {
	...
	pr1.On("SendUserMessage", p1, actor.WrapEnvelope(&actor.PoisonPill{})).Once()

	p2 := system.NewLocalPID("p2")
	c := new(mockContext)
	c.On("Envelope").Return(actor.WrapEnvelope(&RemoveRoutee{PID: p1}))
...
func TestPoolRouterActor_Receive_BroadcastMessage(t *testing.T) {
	...
	c := new(mockContext)
	c.On("Envelope").Return(BroadcastMessageEnvelope(actor.WrapEnvelope("hi")))
