...
func TestRouterSendsUserMessageToChild(t *testing.T) {
	...
	system.Root.Send(routerPID, actor.WrapEnvelope("hello"))
	system.Root.Send(routerPID, actor.WrapEnvelopWithSender("hello", routerPID))
...
func (m *testRouterState) RouteMessage(message *actor.MessageEnvelope) {
	m.Called(message)
